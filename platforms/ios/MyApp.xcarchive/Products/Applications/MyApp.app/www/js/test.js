...
//		CanBtn.addEventListener('touchstart', this.onCancel);
	...
	onSave: function () {
		...
		try{
			...
			if (!(name && 0xff)) {
//				app.generateMessage("#ReviewModal div.content","bad","Please enter a name of Item");
				app.generateMessage("#ReviewModal form.input-group","bad","Please enter a name of Item");
				return;
			}
			
			if(!(imagepath && 0xff)){
			   	app.generateMessage("#ReviewModal div.content","bad","Please take a picture for Item")
			   	return;
			   }
			...
			if (review === null) {
				...
				throw new Error("Create Object Error.Try it agai");
			}
			review.name = name;
	...
	onSuccess: function (imageData) {
		...
		let picbut = document.querySelector("#ReviewModal.btn.btn-primary");
		picbut.style.visibility="hidden";
...
const HTMLHANDLER = {
	...
	replaceBut:function(img){
		...
		let but = document.getquerySelector("#ReviewModal button.btn.btn-primary.btn-block");
		but.style.display = "none";
		
		let modalpad = document.getquerySelector("#ReviewModal p.content-padded");
		modalpad.insertBefore(img,modalpad.childElement(1));
